...
function tryLockLandscape(): void {
    // why: ScreenOrientation.lock is not in the stable TypeScript lib yet
...
export function toggleFs(): void {
    if (inNativeFs()) {
        void (document.exitFullscreen || (document as { webkitExitFullscreen?: () => void }).webkitExitFullscreen)?.call(
            document,
        );
        return;
    }
    ...
    if (!el) return;
    const req =
        el.requestFullscreen || (el as { webkitRequestFullscreen?: (opts: object) => Promise<void> }).webkitRequestFullscreen;
    if (req && !isIOS) {
